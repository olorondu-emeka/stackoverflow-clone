...
import GetQuestion from '../../../src/core/useCases/question/GetQuestion';
import { FinalResponse } from '../../../src/core/definitions/CommonTypes';

const QuestionDataGateway = new QuestionGateway();
const GetQuestionUC = new GetQuestion(QuestionDataGateway);

describe('Unit Test -- GetQuestion', () => {
  afterEach(() => {
  ...
  it('should throw a 404 error for a non-existent question', async () => {
    ...
    const response: FinalResponse = await GetQuestionUC.execute(1);
  ...
  it('should successfully retrieve a question', async () => {
    const response: FinalResponse = await GetQuestionUC.execute(1);
    const { statusCode, data } = response;
